...
async function server(options={}) {
  ...
  const app=new App({purchaseRepository:options.purchaseRepository || {products:{},list:async()=>({records:[],total:0}),locations:async()=>({options:[],total:0})},sessionStore:new MySqlSessionStore(pool),supplierRepository:options.supplierRepository || {list:async()=>({records:[],total:0})},productRepository:options.productRepository || {list:async()=>({records:[],total:0}),options:async()=>({options:[],total:0})}});
