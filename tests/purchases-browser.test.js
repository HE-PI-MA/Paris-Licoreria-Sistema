...
test('U025: Compras en navegador',{skip:process.env.PARIS_UI_BROWSER_TESTS!=='1',timeout:120000},async t=>{
 ...
 try{
  ...
  await t.test('listado, búsqueda, error y reintento',async()=>{
   await page.goto(app.base+'/compras');await page.locator('#purchases-table tr[data-row-index]').waitFor();await shot('u025-listado');
  ...
  await t.test('autocompleta existentes, agrega/edita/quita sin guardar y conserva la equivalencia',async()=>{
   await page.locator('[data-module-primary]').click();await choose('Nombre o empresa','DISTRIB','DISTRIBUIDORA FICTICIA');
  ...
  await t.test('alta inline de producto y presentación; doble envío y respuesta perdida no duplican',async()=>{
   ...
   await modal().locator('tr[data-row-index="1"]').waitFor();assert.equal(await modal().locator('output').textContent(),'Total: Bs 60,30');await shot('u025-nueva-compra');
   ...
   assert.equal(posts.length,2);assert.equal(posts[0].key,posts[1].key);assert.equal(repo.pool.data.purchases.length,2);assert.equal(repo.pool.data.suppliers.length,2);assert.equal(repo.pool.data.products.length,2);assert.equal(repo.pool.data.lines.at(-2).baseQuantity,'24.000');assert.equal(repo.pool.data.presentations.at(-1).barcode,'aBc-Compra');
  ...
  await t.test('detalle confirmado y diseño móvil sin desbordar la página',async()=>{
   await page.getByRole('button',{name:'Acciones del registro 2',exact:true}).click();await page.getByRole('menuitem',{name:'Ver detalle',exact:true}).click();await page.getByRole('dialog',{name:'Compra N.º 2',exact:true}).waitFor();await shot('u025-detalle');await page.getByRole('button',{name:'Cerrar',exact:true}).click();
   await page.locator('.app-toast[data-kind=success]').waitFor({state:'detached'});await page.setViewportSize({width:390,height:844});await page.locator('[data-module-primary]').click();await shot('u025-movil');
