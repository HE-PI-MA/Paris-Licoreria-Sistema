...
test('U029: Compras en navegador',{skip:process.env.PARIS_UI_BROWSER_TESTS!=='1',timeout:120000},async t=>{
 ...
 const page=await browser.newPage({viewport:{width:1440,height:1100},locale:'es-BO'});page.setDefaultTimeout(7000);const errors=[];page.on('pageerror',e=>errors.push(e.message));
 ...
 const menu=async(label)=>{await modal().locator('tr[data-row-index]').first().getByRole('button',{name:/Acciones del registro/}).click();await page.getByRole('menuitem',{name:label,exact:true}).click();};
 ...
 try{
  ...
  await t.test('listado, búsqueda, error y reintento',async()=>{
   await page.goto(app.base+'/compras');await page.locator('#purchases-table tr[data-row-index]').waitFor();await shot('u029-listado');
  ...
  await t.test('texto libre, Escape y descarte no crean registros',async()=>{
   ...
   await modal().getByRole('combobox',{name:'Ubicación *',exact:true}).fill('caja dos');await modal().getByLabel('Observación',{exact:true}).click();
   assert.equal(await modal().getByRole('combobox',{name:'Ubicación *',exact:true}).inputValue(),'CAJA DOS');
   await shot('u029-ubicacion-nueva');assert.equal(repo.pool.data.locations.length,1);
  ...
  await t.test('autocompleta existentes, agrega/edita/quita sin guardar y conserva la equivalencia',async()=>{
   ...
   assert.equal(await modal().getByRole('combobox',{name:'Categoría *',exact:true}).getAttribute('placeholder'),'Ej.: Gaseosas');
   assert.equal(await modal().getByRole('combobox',{name:'¿Cómo lo compras?',exact:true}).getAttribute('placeholder'),'Ej.: Botella o paquete de 6');
   const headerBoxes=await Promise.all(['supplierText','phone','locationIdText','observation'].map(name=>modal().locator('[name='+name+']').boundingBox()));
   assert.ok(headerBoxes.every(b=>Math.abs(b.y-headerBoxes[0].y)<1),'Proveedor e ingreso deben compartir fila');
   await shot('u029-formulario');await choose('Nombre o empresa','DISTRIB','DISTRIBUIDORA FICTICIA');
   ...
   await choose('Ubicación *','ALMA','ALMACÉN DE PRUEBA');await choose('Producto','CERVE','CERVEZA FICTICIA');
   await page.waitForFunction(()=>document.querySelector('[name=categoryId]').disabled);assert.equal(await modal().locator('[name=unitId]').evaluate(n=>n.disabled),true);
   await choose('¿Cómo lo compras?','PAQ','PAQUETE DE 6');await page.waitForFunction(()=>document.querySelector('[name=factor]').value==='6.000');
   ...
   await menu('Ver detalle');const details=page.getByRole('dialog',{name:'Detalle del producto comprado',exact:true});await details.getByText('Cantidad que ingresa',{exact:true}).waitFor();await details.getByRole('button',{name:'Cerrar',exact:true}).click();
  ...
  await t.test('alta inline de producto y presentación; doble envío y respuesta perdida no duplican',async()=>{
   ...
   await modal().getByRole('combobox',{name:'Ubicación *',exact:true}).fill('heladera');await modal().getByLabel('Observación',{exact:true}).click();
   assert.equal(await modal().getByRole('combobox',{name:'Ubicación *',exact:true}).inputValue(),'HELADERA');
   await modal().getByRole('combobox',{name:'Producto',exact:true}).fill('GALLETAS NUEVAS');await modal().getByRole('combobox',{name:'Categoría *',exact:true}).fill('dulces nuevos');await choose('¿Cómo lo cuentas? *','UNI','UNIDAD');
   assert.equal(await modal().getByRole('combobox',{name:'Categoría *',exact:true}).inputValue(),'DULCES NUEVOS');assert.equal(repo.pool.data.categories.length,1);
   await modal().getByRole('combobox',{name:'¿Cómo lo compras?',exact:true}).fill('CAJA DE 12');await modal().locator('[name=factor]').fill('12');await modal().locator('[name=price]').fill('30');
   ...
   await menu('Editar');assert.equal(await modal().getByRole('combobox',{name:'Categoría *',exact:true}).inputValue(),'DULCES NUEVOS');
   await modal().getByRole('button',{name:'Actualizar producto',exact:true}).click();
   await choose('Producto','GALL','GALLETAS NUEVAS (NUEVO EN ESTA COMPRA)');
   assert.equal(await modal().getByRole('combobox',{name:'Categoría *',exact:true}).inputValue(),'DULCES NUEVOS');await choose('¿Cómo lo compras?','CAJA','CAJA DE 12 (NUEVA EN ESTA COMPRA)');
   ...
   await modal().locator('tr[data-row-index="1"]').waitFor();assert.equal(await modal().locator('output').textContent(),'Total: Bs 60,30');await shot('u029-nueva-compra');
   ...
   assert.equal(posts[0].body.lines[0].product.categoryName,'DULCES NUEVOS');assert.deepEqual(posts[0].body.lines[0].product,posts[0].body.lines[1].product);
   assert.equal(repo.pool.data.categories.length,2);assert.equal(repo.pool.data.products.at(-1).categoryId,2);
  ...
  await t.test('detalle confirmado y diseño móvil sin desbordar la página',async()=>{
   await page.getByRole('button',{name:'Acciones del registro 2',exact:true}).click();await page.getByRole('menuitem',{name:'Ver detalle',exact:true}).click();await page.getByRole('dialog',{name:'Compra N.º 2',exact:true}).waitFor();await shot('u029-detalle');await page.getByRole('button',{name:'Cerrar',exact:true}).click();
   await page.locator('.app-toast[data-kind=success]').waitFor({state:'detached'});
   await page.locator('[data-module-primary]').click();await choose('Ubicación *','HELA','HELADERA');
   assert.equal(await modal().locator('[name=locationId]').inputValue(),'2');
   await choose('Categoría *','DUL','DULCES NUEVOS');assert.equal(await modal().locator('[name=categoryId]').inputValue(),'2');
   ...
   await page.setViewportSize({width:390,height:844});await page.locator('[data-module-primary]').click();await shot('u029-movil');
   ...
   await modal().locator('[name=expiresOn]').scrollIntoViewIfNeeded();await shot('u029-movil-campos');
