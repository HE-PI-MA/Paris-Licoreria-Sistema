...
const definitions = {
  ...
  productos: { contentView: 'productos/content', enabled: true, hidePrimary: true, filterMode: 'inline-category', contentVariant: 'listing' },
...
function forPage(page) {
  ...
  return {
    ...definitions[page.id],
    id: page.id,
    searchPlaceholder: page.id === 'inventario' ? 'Ej.: Coca-Cola o maní…' : page.id === 'compras' ? 'Producto o número de compra…' : page.id === 'productos' ? 'Nombre del producto o código de barras…' : page.id === 'proveedores' ? 'Nombre, contacto, teléfono o NIT…' : `Buscar en ${page.label.toLocaleLowerCase('es')}…`,
    controlsHelp: page.id === 'productos' ? 'Los productos se crean al registrar Compras; aquí administras sus datos y formas de venta.' : 'Filtra los registros ficticios de esta demostración.',
