...
function forPage(page) {
  ...
  return {
    ...definitions[page.id],
    id: page.id,
    searchPlaceholder: page.id === 'productos' ? 'Nombre del producto o código de barras…' : page.id === 'proveedores' ? 'Nombre, contacto, teléfono o NIT…' : `Buscar en ${page.label.toLocaleLowerCase('es')}…`,
    ...
    message: page.id === 'demo-componentes'
      ...
      : ['productos', 'proveedores'].includes(page.id) ? '' : 'Los controles se habilitarán cuando este módulo esté disponible.'
  };
}
