/** U044: listado de Compras enfocado en fecha, total, usuario y productos ingresados. */
(() => {
  'use strict';
  const UI = window.ParisUI, P = window.ParisPurchases;
  class PurchasesPage {
    constructor(element) {
      this.api = new UI.CatalogApi('/api/compras');
      this.events = new AbortController();
      this.dialogs = new Set();
      this.notifications = UI.NotificationCenter.shared();
      this.table = new UI.DataTable({
        container: element, caption: 'Compras', mode: 'scroll', numbered: true, pageSize: 50,
        columns: [
          { key: 'date', label: 'Fecha', sortable: true },
          { key: 'total', label: 'Total (Bs)', type: 'price', sortable: true },
          { key: 'user', label: 'Registrado por', priority: 1 }
        ],
        sort: { key: 'date', direction: 'desc' },
        load: p => this.api.list(p),
        actionDisplay: 'menu',
      ...
      this.filters = new UI.FilterBar({
        ...
        mode: 'inline',
        onChange: q => this.table.setQuery(q)
      });
      document.querySelector('[data-module-primary]').addEventListener('click', event => {
        const form = new P.PurchaseForm({ api: this.api, opener: event.currentTarget, onSaved: result => this.saved(result) });
        this.track(form.modal);
  ...
  P.PurchasesPage = PurchasesPage;
  const host = document.getElementById('purchases-table');
  if (host) new PurchasesPage(host);
})();
