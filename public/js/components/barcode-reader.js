...
(() => {
  ...
  class BarcodeScanner {
    constructor({ onRead, opener }) {
      ...
      const photo = UI.Button.create({ label: 'Foto del código', icon: 'camera' }), input = UI.element('input'); input.type = 'file'; input.accept = 'image/jpeg,image/png,image/webp'; input.setAttribute('capture', 'environment'); input.hidden = true;
      toolbar.append(this.start, photo, input); content.append(this.video, this.help, toolbar);
      ...
      if (!window.isSecureContext || !navigator.mediaDevices?.getUserMedia) this.help.textContent = 'Para cámara en vivo abre el sistema por HTTPS. Puedes usar Foto del código o escribirlo con el lector.';
    }
    decoder() {
    ...
    async camera() {
      ...
      try {
        ...
        const controls = await this.decoder().decodeFromStream(stream, this.video, result => { if (result) this.complete(result.getText(), generation); });
    ...
    async photo(file) {
      ...
      try { const canvas = await UI.ImageFile.canvas(file, 1920); if (generation !== this.generation || this.modal.destroyed) return; this.complete(this.decoder().decodeFromCanvas(canvas).getText(), generation); }
  ...
  class BarcodeField {
    constructor({ input, signal, onRead = () => {}, label = 'Leer código' }) {
      this.input = input; this.onRead = onRead;
      const row = UI.element('div', 'app-input-action'); input.replaceWith(row); row.append(input);
      this.button = UI.Button.create({ label, icon: 'barcode', iconOnly: true }); row.append(this.button);
