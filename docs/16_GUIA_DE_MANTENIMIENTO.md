...
Desde [U026](38_UBICACIONES_Y_FORMULARIO_U026.md), la ubicación usa el mismo SearchSelect con texto libre. PurchaseInput acepta locationId o locationName; conservar la forma normalizada anterior por ID para no invalidar los reintentos U025. PurchaseService crea o reutiliza el destino dentro de OperationStore y PurchaseRepository conserva el índice único de ubicacion. No crear lugares al escribir ni al agregar filas al borrador. La variante app-form-grid--compact pertenece a forms.css; no añadir CSS de página para esta distribución.

Desde [U028](40_NOTIFICACIONES_GLOBALES_U028.md), los resultados de acciones, incluidos errores, usan NotificationCenter con la duración global de dos segundos. No crear Message dentro del listado de presentaciones para mostrar esos resultados. Message sigue siendo el presentador compartido para validaciones junto a campos, estados de carga y errores de carga de tablas que incluyen Reintentar. Los colores y la posición pertenecen a messages.css; no crear estilos ni temporizadores en cada página.
